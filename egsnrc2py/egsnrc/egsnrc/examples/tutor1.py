...
#
#   Darcy Mason, 2021: Converted to Python version
#
# #############################################################################
...
# ---------------------------------------------------------------------
params.MXMED = 1   # only 1 medium in the problem(default 10)
params.MXREG = 3   # only 3 geometric regions (default 2000)
params.MXSTACK = 15 # less than 15 particles on stack at once
...
MED = np.array([0, 0, 1, 0])
# /MED(1),MED(3)/=0;MED(2)=1 # vacuum in regions 1 and 3, Ta in region 2

# %E  # tutor1.mortran

ECUT = np.array([0, 0, 1.5])
PCUT = np.array([0, 0, 0.1])
...
HATCH()     #     pick up cross section data for TA
